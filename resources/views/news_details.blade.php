...
<body class="">
    <main class="pt-33 pb-20 pl-3 pr-3">
        <section class="max-w-[900px] mx-auto bg-white rounded-[20px] border-2 border-black p-6">
            <img class="rounded-[20px] w-full mb-4 h-96 object-cover" src="{{ $news->image_url ?: 'https://picsum.photos/600/400?random=' . $news->id }}"/>
            ...
            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 sm:gap-6 mb-6">
                ...
                <div class="flex-1">
                    <h1 class="text-blue-800 font-extrabold text-3xl leading-tight">{{ $news->title }}</h1>
                    <p class="text-gray-400 text-sm mt-2">Written By: {{ $news->user->name }}</p>
                </div>
            ...
            <p class="text-blue-800 text-base leading-tight mb-2 font-bold">Full Article:</p>
            <p class="text-blue-800 text-sm leading-tight mb-6 text-justify">{{ $news->content }}</p>
